...
export default function QRAttendanceScanner({ classes = [], onNavigate }: { classes?: any[]; onNavigate?: (view: string) => void }) {
    ...
    const handleScan = useCallback(async (decodedText: string) => {
        ...
        try {
            ...
            setLastResult({ type: 'success', message: `✓ ${result.student_name} marked present` });
        } catch (err: any) {
    ...
    const startScanner = async () => {
        try {
            ...
            setLastResult({ type: 'error', message: 'Camera access denied. Please allow camera permissions.' });
        }
    };
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-zinc-500 mt-1">Scan student ID cards to mark attendance</p>
                </div>
            ...
            <div className="grid grid-cols-3 gap-4">
                <div className="p-4 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-2xl text-center">
                    ...
                    <p className="text-xs font-bold text-emerald-600/70 uppercase tracking-widest mt-1">Marked Present</p>
                </div>
                <div className="p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl text-center">
                    ...
                    <p className="text-xs font-bold text-amber-600/70 uppercase tracking-widest mt-1">Already Marked</p>
                </div>
                <div className="p-4 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 rounded-2xl text-center">
                    ...
                    <p className="text-xs font-bold text-indigo-600/70 uppercase tracking-widest mt-1">Total Scans</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden shadow-sm">
                    <div className="p-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
                        ...
                        >
                            {scanning ? (
                                <span className="flex items-center gap-2"><CameraOff className="w-4 h-4" /> Stop</span>
                            ) : (
                                <span className="flex items-center gap-2"><Camera className="w-4 h-4" /> Start Scanning</span>
                            )}
                        </button>
                    </div>
                    ...
                    <div className="relative">
                        ...
                        {!scanning && (
                            <div className="flex flex-col items-center justify-center py-20 px-8 text-center bg-zinc-50 dark:bg-zinc-800/50">
                                ...
                                <p className="text-zinc-400 text-sm mt-1">Click "Start Scanning" to open the camera and begin scanning QR codes</p>
                            </div>
                        )}
                    ...
                    <div className="max-h-[420px] overflow-y-auto divide-y divide-zinc-100 dark:divide-zinc-800">
                        {scannedList.length === 0 ? (
                            <div className="py-16 text-center">
                                ...
                                <p className="text-zinc-400 text-sm">No students scanned yet</p>
                                <p className="text-zinc-300 dark:text-zinc-600 text-xs mt-1">Scan a QR code to start</p>
                            </div>
                        ) : (
                            scannedList.map((s, i) => (
                                <div key={`${s.id}-${i}`} className="px-4 py-3 flex items-center gap-3 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                                    ...
                                    <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-lg ${s.status === 'success' ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600' :
                                        ...
                                        {s.status === 'success' ? 'Present' : s.status === 'duplicate' ? 'Already' : 'Error'}
                                    </span>
                                </div>
                            ))
                        )}
